    ...
    parseTree(): void {
        ...
        function callback(this: ApplicationInsightsExplorerExplorerProvider, error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                function mergeStep(value: any[]) {
                    const groupedByCorrelationId = groupByMultilevel(value, [
                        "customDimensions",
                        "CorrelationId",
                    ]);
                    let processed: any[] = [];
                    // Iterate over the grouped data
                    for (const group of groupedByCorrelationId) {
                        const logMessages = groupByMultilevel(group.values, [
                            "cloud",
                            "roleInstance",
                        ]);
